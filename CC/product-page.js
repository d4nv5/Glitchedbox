...
variantItems.forEach(item => {
    item.addEventListener('click', () => {
        ...
        // Cambiar TODAS las imágenes del slider a la variante seleccionada
        // Patrón: gallina_blanca_1.webp, _2, _3 → gallina_${color}_1.webp, _2, _3
        sliderImages.forEach((img, idx) => {
            img.src = `images/gallina_${variantName}_${idx + 1}.webp`;
        });
        
        // Volver al primer slide al cambiar variante
        sliderIndex = 1;
        if (sliderTrack) {
            const slideWidth = sliderTrack.children[0]?.offsetWidth || sliderTrack.offsetWidth;
            sliderTrack.style.transition = 'none';
            sliderTrack.style.transform = `translateX(${-slideWidth * sliderIndex}px)`;
        }
    });
...
/* ══════════════════════════════════════════════════════════════
   TEXT SCRAMBLE — efecto hover tipo Ray-Ban EXE
   Se aplica a .product-title y .product-description automáticamente.
   Reutilizable: cualquier elemento con [data-scramble] también lo recibe.

   Comportamiento:
   - Al hacer hover, los caracteres se barajan con glyphs aleatorios
     y se van resolviendo de izquierda a derecha rápidamente
   - Duración total ~600ms para no interrumpir la lectura
   - Se cancela inmediatamente si el mouse sale antes de terminar
   - No modifica el DOM original: usa un span interno para el output
══════════════════════════════════════════════════════════════ */
(function () {
  'use strict';

  /* Charset de glyphs — mezcla de símbolos, katakana y chars técnicos */
  const CHARS = '!<>-_\\/[]{}—=+*^?#@|~`$%&ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz01アイウエオカキクケコサシスセソタチツテトナニヌネノ';

  /* ── Clase TextScramble ── */
  class TextScramble {
    constructor(el) {
      this.el        = el;
      this.original  = el.textContent;   /* texto real — fuente de verdad */
      this.frame     = 0;
      this.rafId     = null;
      this.queue     = [];
      this.cancelled = false;

      /* Span interno para el output — preserva el DOM del elemento */
      this.output = document.createElement('span');
      this.output.setAttribute('aria-hidden', 'true');
      this.output.className = 'scramble-output';
      /* Mostrar texto original desde el inicio — sin ghost trick */
      this.output.textContent = this.original;

      /* Limpiar texto del elemento y meter el output */
      el.textContent = '';
      el.appendChild(this.output);
      el.setAttribute('aria-label', this.original);

      /* Velocidad: caracteres por frame — rápido para no bloquear lectura */
      /* ~600ms total para textos largos, ~200ms para títulos cortos */
    }
    ...
    scramble() {
      ...
      /* Cuántos frames dura el efecto total: ~ len * 1.5, mín 12, máx 36 */
      const totalFrames = Math.min(Math.max(Math.floor(len * 5), 60), 180);

      /* Cola: para cada caracter, asignar en qué frame se "resuelve" */
      this.queue = Array.from(text).map((char, i) => ({
        from:  char,
        to:    char,
        start: Math.floor((i / len) * totalFrames * 0.7), /* stagger izq→der */
        end:   Math.floor((i / len) * totalFrames * 0.7) + Math.floor(Math.random() * 6) + 3,
        char:  '',
      }));
    ...
    update() {
      ...
      /* Throttle: solo actualizar cada 3 ticks de RAF (~50ms por paso) */
      ...
      for (const item of this.queue) {
        if (this.frame >= item.end) {
          ...
          if (this.frame >= item.start) {
            /* En scramble — glyph aleatorio */
            item.char = CHARS[Math.floor(Math.random() * CHARS.length)];
            output   += `<span class="scramble-char">${item.char}</span>`;
          } else {
            /* Todavía no empezó — mostrar el original o espacio */
            output += item.from === ' ' ? ' ' : `<span class="scramble-char">${item.from}</span>`;
          }
        }
      }
    ...
    cancel() {
      ...
      /* Restaurar texto limpio inmediatamente */
      this.output.innerHTML = this.original;
    }
  }

  /* ── Init: aplicar a .product-title, .product-description, [data-scramble] ── */
  function initScramble() {
    ...
    selectors.forEach(sel => {
      document.querySelectorAll(sel).forEach(el => {
        ...
        el.addEventListener('mouseleave', () => fx.cancel());

        /* Touch: pequeño scramble rápido al tocar (no bloquea scroll) */
        el.addEventListener('touchstart', () => {
          fx.scramble();
          setTimeout(() => fx.cancel(), 400);
        }, { passive: true });
      });
    });
  }
  ...
  /* Exponer para re-init tras cambio de producto dinámico */
  window.initTextScramble = initScramble;
